...
/**
 * <p>Reads the EDF header to the EDFP object and 
 *    its annotations' and record's simblings </p>
 * @param inlet a pointer to the file
 * @param debug <p>false by default, it prints header 
 *              info on screen using the YAML format</p>.
 */
 ...
 * @param debug <p>false by default, determines if it should or not 
                write stuff on terminal</p>.
...
/**
 * Gets the annotations from the file, if it contains so.
 */
const char* EDFP::get_annotations()
{
	return (isEDFP)? annotations.get_annotations() : NULL;
}
